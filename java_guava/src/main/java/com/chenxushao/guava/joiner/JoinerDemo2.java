...
public class JoinerDemo2 {
	...
	public static List<String> languages = Arrays.asList("Java", "Haskell",
			"Scala", "Brainfuck");

	@Test
	public void shouldJoinWithCommas() throws Exception {
		assertThat(Joiner.on(",").join(languages)).isEqualTo(
				"Java,Haskell,Scala,Brainfuck");
	}

	public void shouldThrowNullPointerException() throws Exception {
		assertThat(Joiner.on(",").join(countriesWithNullValue)).isEqualTo(
				"Poland,Brasil,Ukraine,null,England,Croatia");
	}

	public static List<String> countriesWithNullValue = Arrays.asList("Poland",
			"Brasil", "Ukraine", null, "England", "Croatia");
	...
	public void shouldJoinWithCommasAndReplaceNullsWithWordNothing()
			throws Exception {
		assertThat(
				Joiner.on(",").useForNull("NONE").join(countriesWithNullValue))
	...
	public void shouldJoinMap() throws Exception {
		assertThat(
				Joiner.on(" | ").withKeyValueSeparator(" -> ")
						.useForNull("Unknown").join(numbersWords)).isEqualTo(
