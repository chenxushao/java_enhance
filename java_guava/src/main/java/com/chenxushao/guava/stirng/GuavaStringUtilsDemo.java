...
public class GuavaStringUtilsDemo {
	...
	public void joiner() {
		...
		// 忽略Null值。
		String[] fantasyGenres = { "Space Opera", null, "Horror",
				"Magic realism", null, "Religion" };
		...
		// 将Null值转换为特定字符串.
		String[] fantasyGenres2 = { "Space Opera", null, "Horror",
				"Magic realism", null, "Religion" };
		joined = Joiner.on(", ").useForNull("NULL!!!").join(fantasyGenres2);
		assertThat(joined)
				.isEqualTo(
						"Space Opera, NULL!!!, Horror, Magic realism, NULL!!!, Religion");
		...
		joined = Joiner.on(", ").skipNulls().appendTo(sb, fantasyGenres)
				.toString();
		assertThat(joined)
				.isEqualTo(
						"Append StringBulder demo: Space Opera, Horror, Magic realism, Religion");
	...
	public void splitter() {
		...
		assertThat(result).containsSequence("Space Opera", "Horror",
				"Magic realism", "Religion");
	}
	...
	public void caseFormatByGuava() {
		...
		assertThat(CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, name))
				.isEqualTo("SpaceSize");
		assertThat(CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL, name))
				.isEqualTo("spaceSize");
		assertThat(
				CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.LOWER_UNDERSCORE,
						name)).isEqualTo("space_size");
		assertThat(
				CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.LOWER_HYPHEN, name))
				.isEqualTo("space-size");
	}

	@Test
	public void otherUtils() {
		// 直接getBytes, 无需catch UnsupportedEncodingException, JDK7
		// 有相应的StandardCharsets
