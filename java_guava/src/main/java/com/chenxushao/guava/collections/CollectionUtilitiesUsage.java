...
public class CollectionUtilitiesUsage {
	...
	static void useIterables() {
		Iterable<Integer> concatenated = Iterables.concat(Ints.asList(1, 2, 3),
				Ints.asList(4, 5, 6), Ints.asList(7, 8, 9));
	...
	static void useSets() {
		...
		Set<List<String>> cartesianProduct = Sets.<String> cartesianProduct(
				animals, fruits);
	...
	static void useMaps() {
		List<String> strings = Lists.newArrayList("hello", "world!", "lan",
				"bo", "shansun");

		// 一个对象集合，我们知道他们有一些独特（可相互区分、唯一性）的属性，并且我们希望通过这些属性的值找到相关的对象时
		// 可以选择使用Maps.uniqueIndex去生成一个key为根据指定属性生成的唯一值
		ImmutableMap<Integer, String> uniqueIndex = Maps.uniqueIndex(strings,
		...
		ImmutableMap<String, Integer> left = ImmutableMap.<String, Integer> of(
				"a", 1, "b", 2, "c", 3);
		ImmutableMap<String, Integer> right = ImmutableMap
				.<String, Integer> of("b", 2, "c", 4, "d", 5);
	...
	static void useMultimaps() {
		ImmutableSet<String> immutableSet = ImmutableSet.of("zero", "one",
				"two", "three", "four");
		...
		ImmutableListMultimap<Integer, String> multimap = Multimaps.index(
				immutableSet, lengthFunc);
		...
		HashMultimap<Integer, String> inverse = Multimaps.invertFrom(multimap2,
