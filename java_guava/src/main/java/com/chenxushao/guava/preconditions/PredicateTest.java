...
public class PredicateTest {
	...
	public void shouldUseCustomPredicate() throws Exception {
		...
		// when
		boolean allCountriesSpecifyCapitalCity = Iterables.all(Lists
				.newArrayList(Country.POLAND, Country.BELGIUM,
	...
	public void shouldComposeTwoPredicates() throws Exception {
		...
		Predicate<Country> composedPredicate = Predicates.and(
				fromEuropePredicate, populationPredicate);

		// when
		Iterable<Country> filteredCountries = Iterables.filter(
				Country.getSomeCountries(), composedPredicate);

		// then
		assertThat(filteredCountries)
				.contains(Country.BELGIUM, Country.ICELAND);
	}
	...
	public void shouldCheckPattern() throws Exception {
		...
		// given
		Predicate<CharSequence> twoDigitsPredicate = Predicates
				.containsPattern("\\d\\d");
	...
	public void shouldFindObjectInCollection() throws Exception {
		...
		// given
		Predicate elevenInCollectionPredicate = Predicates.in(Arrays.asList(
				11L, 22L, 33L, 44L));
