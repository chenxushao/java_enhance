...
public class MultisetTest extends TestCase {
	 ...
	 * 它继承的是Collection<E>接口，你可以向Multiset中添加重复的元素， Multiset会对添加的元素做一个计数。
	 * 
	 * 它本质上是一个Set加一个元素计数器。
	 */
	...
	public void test() {
		...
		Iterable<String> words = Splitter.onPattern("[^a-z]{1,}")
				.omitEmptyStrings().trimResults().split(sentences);
		...
		for (Object element : multiset.elementSet()) {
			System.out
					.println((String) element + ":" + multiset.count(element));
		}
	}
